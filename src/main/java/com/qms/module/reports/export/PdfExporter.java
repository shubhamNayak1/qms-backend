...
public class PdfExporter {
    ...
    public byte[] exportCapa(List<CapaReportRow> rows, ReportSummary summary, String title) {
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            addReportHeader(doc, title, summary);
            if (summary != null) addSummaryTable(doc, summary);
            doc.add(Chunk.NEWLINE);

            float[] widths = {1.5f, 3f, 1.2f, 1f, 1.2f, 1.2f, 1.5f, 1.5f, 1.2f, 1.2f, 1f, 1f};
            PdfPTable table = createTable(widths,
                    "Record #", "Title", "Status", "Priority", "Source", "CAPA Type",
                    "Department", "Assigned To", "Due Date", "Closed Date", "Overdue?", "Age (Days)");

            for (int i = 0; i < rows.size(); i++) {
                CapaReportRow r = rows.get(i);
                Color rowBg = i % 2 == 0 ? WHITE : ALT_ROW_BG;
                Color bg    = r.isOverdue() ? ALERT_BG : rowBg;
                addCell(table, r.getRecordNumber(), bg);
                addCell(table, truncate(r.getTitle(), 60), bg);
                addCell(table, r.getStatus(), bg);
                addCell(table, r.getPriority(), bg);
                addCell(table, r.getSource(), bg);
                addCell(table, r.getCapaType(), bg);
                addCell(table, r.getDepartment(), bg);
                addCell(table, r.getAssignedToName(), bg);
                addCell(table, fmt(r.getDueDate()), bg);
                addCell(table, fmt(r.getClosedDate()), bg);
                addCell(table, r.isOverdue() ? "YES" : "No", bg);
                addCell(table, String.valueOf(r.getAgeInDays()), bg);
            }
            doc.add(table);
            doc.close();
    ...
    public byte[] exportDeviation(List<DeviationReportRow> rows, ReportSummary summary, String title) {
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            float[] widths = {1.5f, 3f, 1.2f, 1f, 1.2f, 1.5f, 1.5f, 1.5f, 1f, 1.2f, 1.2f, 1f};
            PdfPTable table = createTable(widths,
                    "Record #", "Title", "Status", "Priority", "Type", "Product Batch",
                    "Process Area", "Department", "CAPA Req?", "Regulatory?", "Due Date", "Overdue?");

            for (int i = 0; i < rows.size(); i++) {
                DeviationReportRow r = rows.get(i);
                Color bg = r.isOverdue() ? ALERT_BG : (i % 2 == 0 ? WHITE : ALT_ROW_BG);
                addCell(table, r.getRecordNumber(), bg); addCell(table, truncate(r.getTitle(), 60), bg);
                addCell(table, r.getStatus(), bg); addCell(table, r.getPriority(), bg);
                addCell(table, r.getDeviationType(), bg); addCell(table, r.getProductBatch(), bg);
                addCell(table, r.getProcessArea(), bg); addCell(table, r.getDepartment(), bg);
                addCell(table, bool(r.getCapaRequired()), bg); addCell(table, bool(r.getRegulatoryReportable()), bg);
                addCell(table, fmt(r.getDueDate()), bg); addCell(table, r.isOverdue() ? "YES" : "No", bg);
            }
            doc.add(table);
            doc.close();
            return out.toByteArray();
    ...
    public byte[] exportIncident(List<IncidentReportRow> rows, ReportSummary summary, String title) {
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            float[] widths = {1.5f, 3f, 1.2f, 1f, 1.2f, 1f, 1.5f, 1.5f, 1.2f, 1f, 1.2f, 1f};
            PdfPTable table = createTable(widths,
                    "Record #", "Title", "Status", "Priority", "Type", "Severity",
                    "Location", "Department", "Occurrence", "Injury?", "Due Date", "Overdue?");
            ...
            for (int i = 0; i < rows.size(); i++) {
                ...
                addCell(table, r.getRecordNumber(), bg); addCell(table, truncate(r.getTitle(), 60), bg);
                addCell(table, r.getStatus(), bg); addCell(table, r.getPriority(), bg);
                addCell(table, r.getIncidentType(), bg); addCell(table, r.getSeverity(), bg);
                addCell(table, r.getLocation(), bg); addCell(table, r.getDepartment(), bg);
                addCell(table, fmt(r.getOccurrenceDate()), bg); addCell(table, bool(r.getInjuryInvolved()), bg);
                addCell(table, fmt(r.getDueDate()), bg); addCell(table, r.isOverdue() ? "YES" : "No", bg);
            }
            doc.add(table);
            doc.close();
